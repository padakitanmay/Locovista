...
export const getAllPendingTours = async (req, res)=>{
    try{
        const pendingTours = await Tour.find({isApproved : false, isRejected : false});

        if(!pendingTours){
            return res.status(404).json({success : false, message : "User not found"})
        }
        return res.status(200).json({success : true, message :"Tours fetched successfully", pendingTours})
    }
    catch(err){
...
export const AcceptTour =  async (req , res)=>{
    try {
        ...
        tour.save();
       return  res.status(200).json({ success: true, message: "Tour Approved Successfully" });
    } catch (error) {
